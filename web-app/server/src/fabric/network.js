...
exports.registerVoter = async function (admin_credentials, voterId, electionId, firstName, lastName, email, password, data) {
  ...
  try {
    ...
    const message = {
      from: 'admin@votarino.com', // Sender address
      to: email,         // List of recipients
      subject: 'votarino - Election Credentials', // Subject line
      html: `<h1>Welcome to votarino!</h1><p>Hey <b>${firstName} ${lastName}</b><br><p>You were registered to the election <b>${electionId}</b><br><p>Here's your <b>voter id</b>: ${voterId} <br><br><p>Here's your <b>password</b>: ${password} <br>Keep it somewhere safe!` // HTML message
    };
...
exports.registerAdmin = async function (firstName, lastName, email, password) {
  ...
  try {
    ...
    const message = {
      ...
      to: email,         // List of recipients
      subject: 'votarino - Election Credentials', // Subject line
      html: `<h1>Welcome to votarino!</h1><p>Hey <b>${firstName} ${lastName}</b>!<br>You were registered as an admin by our super admin, you can create elections and add voters!<br><p>Here's your <b>password</b>: ${password} <br>Keep it somewhere safe!` // HTML message
    };
...
exports.registerSuperAdmin = async function (firstName, lastName, email, password) {
  ...
  try {
    ...
    const message = {
      ...
      to: email,         // List of recipients
      subject: 'votarino - Election Credentials', // Subject line
      html: `<h1>Welcome to votarino!</h1><p>Hey <b>${firstName} ${lastName}</b><br><p>Here's your <b>password</b>: ${password} <br>Keep it somewhere safe!` // HTML message
    };
